####split the text into slices and devide slice into individual parts 
##do sentiment analysis for each person in each documents
rm(list = ls())
wd <- 'E:/workspace'
setwd(wd)
library(syuzhet)
...
##this part is a little trick because you have to do it document by document and change the parameter of loop every time
...
##preprocessing and do sentiment analysis for one person in each slice 
...
jpeg(filename=paste(j,"p.jpeg",sep=''))
barplot(sent.v, main="narrator sentiments in d1 ",type='b')
dev.off
}